    ...
    return (
        <div>
                ...
                <IonToolbar>
                    <div className="flex justify-between items-center px-2 pl-3 text-xl font-bold">
                        ...
                        <div className="flex gap-1">
                            <button
                                className={`flex gap-2 px-2 p-1 items-center w-fit rounded-xl text-sm h-fit ${
                                    view === "month"
                                        ? "bg-accent"
                                        : "bg-white/10"
                                }`}
                            ...
                            <button
                                className={`flex gap-1 px-2 p-1 items-center w-fit rounded-xl text-sm h-fit ${
                                    view === "week"
                                        ? "bg-accent"
                                        : "bg-white/10"
                                }`}
            ...
            <div className="p-4">
                <div className="grid grid-cols-7 text-center text-xs mb-2">
                    {weekDays.map((day, index) => (
                        <div
                            key={index}
                            className={`${
                                getDay() === day ? "text-accent" : "text-white"
                            } font-bold`}
                        >
                            {day}
                        </div>
                ...
                <div
                    className={`grid grid-cols-7 text-center text-lg border-gray-700 ${
                        view === "month" ? "border-0" : "border-b"
                    }`}
                >
                    {(view === "month" ? dates_month : dates_week).map(
                        (date, index) => (
                            <div
                                key={index}
                                className={`${
                ...
                <div
                    className={`w-full justify-center border-b border-gray-700 p-2 font-bold text-xl ${
                        view === "month" ? "flex" : "hidden"
                    }`}
                >
                    {month} {year}
                </div>
